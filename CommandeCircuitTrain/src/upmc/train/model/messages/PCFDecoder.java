...
public class PCFDecoder 
{
	...
	private PCFSwitchElement decodeSwitch(Node node)  throws BadPCFFormatException {
		String idT = node.getAttributes().getNamedItem("id").getTextContent() ;
		String position = node.getAttributes().getNamedItem("pos").getTextContent() ;
		return new PCFSwitchElement(idT,position) ;
	...
	private PCFSwitchEdgeElement decodeSwitchEdge(Node node) throws BadPCFFormatException {
		
		String id = node.getAttributes().getNamedItem("id").getTextContent() ;
		String type = node.getAttributes().getNamedItem("type").getTextContent() ;	
		String branch0 = node.getAttributes().getNamedItem("branch0").getTextContent() ;
		String branch1 = node.getAttributes().getNamedItem("branch1").getTextContent() ;
		String trunk = node.getAttributes().getNamedItem("trunk").getTextContent() ;
		PCFSwitchEdgeElement edge = new PCFSwitchEdgeElement(id,type,trunk,branch0,branch1) ;
		return edge ;
	}
	...
	private void decodeTopography()  throws BadPCFFormatException 
	{
		...
		for (int j = 0 ; j< enfants.getLength() ; j++)
		{
			...
			//modification pour nouvelle topo
			if (edge.getNodeName().equals("sensor-edges"))
				topo.addEdge(this.decodeEdge(edge));
			else
				topo.addSwitchEdge(this.decodeSwitchEdge(edge));
	...
	private void decodeSet()  throws BadPCFFormatException 
	{
		...
		for (int j = 0 ; j< enfants.getLength() ; j++)
		{
			...
			else if (trainOuLight.getNodeName().equals("switch"))
				set.addSwitch(this.decodeSwitch(trainOuLight));			
